...
(define (domain sliding-tile)
  ...
  (:action cheat
    :parameters (?tile ?new-row ?new-col ?old-row ?old-col)
    :precondition (and (IS-TILE ?tile)
                       (IS-ROW ?old-row)
                       (IS-ROW ?new-row)
                       (IS-COLUMN ?old-col)
                       (IS-COLUMN ?new-col)
                       (tile-at ?tile ?old-row ?old-col)
                       (is-blank ?new-row ?new-col))
    :effect (and (not (tile-at ?tile ?old-row ?old-col))
                 (not (is-blank ?new-row ?new-col))
                 (tile-at ?tile ?new-row ?new-col)
                 (is-blank ?old-row ?old-col)))
  )
